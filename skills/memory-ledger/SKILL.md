...
- `references/EXAMPLES.md` — a live ledger maintained across a multi-step Python project. Read it when the expected output format or level of detail is unclear.
...
**Workflows:**
- **Complex Task Execution Flow** (`orch-complex-task`, step 4 of 6): next → `@output-critic` *(optional)*.
- **Knowledge Capture Flow** (`orch-knowledge`, step 1 of 2): next → `@mastermind-logger`.

**Direct handoffs:**
- `@session-transfer` — The task will continue in a new session.
