...
- **Domain Awareness Check**: If the request matches a domain in the *Ecosystem Index* below, follow that ecosystem's workflow map (`@ecosystem-<domain>`, `references/workflows.json`) and build your Dependency Map from its steps. If the ecosystem skill is not installed, use the index as the plan skeleton. Never invent skills that are not listed.
