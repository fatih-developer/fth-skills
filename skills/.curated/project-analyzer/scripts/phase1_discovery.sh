...
echo "--- AŞAMA 1 / PHASE 1: Proje Keşfi / Project Discovery ---"

echo "1. Genel yapı / General structure"
find . -maxdepth 3 -type f | grep -v node_modules | grep -v .git | grep -v dist | sort

echo "2. Dil ve framework tespiti / Language and framework detection"
...
echo "3. Satır sayısı ve dil dağılımı / Line count and language distribution"
find . -type f \( -name "*.ts" -o -name "*.tsx" -o -name "*.js" -o -name "*.py" \
       -o -name "*.go" -o -name "*.rs" -o -name "*.java" -o -name "*.cs" \) \
       -not -path "*/node_modules/*" -not -path "*/.git/*" -not -path "*/dist/*" \
  | xargs wc -l 2>/dev/null | sort -rn | head -30

echo "4. Git geçmişi / Git history (if any)"
...
echo "5. Bağımlılıklar / Dependencies"
...
echo "6. Test dosyaları / Test files"
find . -type f \( -name "*.test.*" -o -name "*.spec.*" -o -name "*_test.*" \) \
       -not -path "*/node_modules/*" | wc -l

echo "7. Ortam ve konfigürasyon / Environment and configuration"
