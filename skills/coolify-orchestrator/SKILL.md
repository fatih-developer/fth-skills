...
description: "Operate self-hosted Coolify: deploy and verify apps, read build logs, provision services and databases, manage per-tenant environment variables, and debug containers, via the coolify-mcp server or the REST API. Use when the user mentions Coolify, a failed deploy, a service that is down or unhealthy, adding a tenant, setting env vars, Traefik, or Turborepo/Bun builds on Coolify."
---
...
2. Env changes take effect only after a restart. Restarting a production service or applying the same change to several tenants is production-impacting: show the list of affected apps and get explicit confirmation (`@checkpoint-guardian` when installed) before restarting
...
After in-container operations are completed, the service must be restarted for changes to apply. Ask for confirmation before restarting a production service, then run:
```bash
# API mode
...
- Obtain explicit user approval before destructive or production-impacting operations: stop, stop_all, delete, restart or redeploy of a production service, bulk env var changes across tenants, and database changes (`@checkpoint-guardian` when installed)
...
**Workflows:**
- **Deploy & Verify Flow** (`web-deploy`, step 2 of 3): next → `@geo-auditor` *(optional)*.
