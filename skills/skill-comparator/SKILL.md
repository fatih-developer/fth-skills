...
**Direct handoffs:**
- `@security-auditor` — A candidate skill will be adopted.

**Handoff contract:** pass results to the next skill through `docs/skill-report/skill-analysis-report.md` with the fields `skill`, `workflow`, `created_at`, `inputs`, `summary`, and `next` (the handoff contract of `@ecosystem-orchestration`). If a next skill is not installed, continue with its step from the ecosystem map, or install it with `npx skills add fatih-developer/fth-skills --skill <name>` after the user agrees.
