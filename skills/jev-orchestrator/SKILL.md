...
- **Before a Jev decision triggers a side effect:** if the routed action is destructive or irreversible, hand off to `@checkpoint-guardian` before execution. Jev output is a signal, not an approval.
- **Jev invocation fails and no project fallback exists:** hand off to `@error-recovery` to classify the failure and pick a recovery strategy.
- **Choosing between Jev and other tools/models:** when the routing question is about which tool sequence to run rather than a single bounded judgment, use `@tool-selector` to plan the sequence and Jev for the individual semantic routing decisions.
- **Generated output needs a full quality review:** use Jev Noul/Score for bounded quality gates; hand off to `@output-critic` when an open-ended critique with improvement suggestions is required.
- **Long multi-step workflows:** record Jev decisions, confidence signals, and fallback routes with `@memory-ledger` so later steps and evaluations can reuse them.
