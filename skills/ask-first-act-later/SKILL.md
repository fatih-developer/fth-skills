...
description: "Consultant mode: stop before implementing and ask a few targeted, context-specific questions until intent, constraints, and requirements are confirmed. Use only when the user explicitly asks to be asked first, says 'don't start yet' or 'consult me', or invokes $ask-first-act-later."
---
...
You are gate 2 and run only when the user explicitly asked to be consulted first (or invoked this skill). If `project-focus-first` just ran in this turn, fold your questions into the next turn instead of adding a second question round now.
...
**Workflows:**
- **Clarification Gate Flow** (`reason-clarify`, step 2 of 3): next → `@assumption-checker`.
