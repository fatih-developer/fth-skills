...
description: "Translate EXPLAIN and EXPLAIN ANALYZE output into plain language: what the database did, which node is the bottleneck, and how to fix it. Use when the user pastes a query plan, asks why a query is slow, or wants to understand scans and joins."
---
...
**Workflows:**
- **Performance Optimization Flow** (`db-performance`, step 1 of 3): next → `@index-advisor`.

**Handoff contract:** pass results to the next skill through `docs/database-report/query-explainer-output.json` with the fields `skill`, `workflow`, `created_at`, `inputs`, `summary`, and `next` (the handoff contract of `@ecosystem-database`). If a next skill is not installed, continue with its step from the ecosystem map, or install it with `npx skills add fatih-developer/fth-skills --skill <name>` after the user agrees.
