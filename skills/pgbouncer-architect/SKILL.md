...
Read `references/orm-compatibility.md` for the section matching the project's driver (Drizzle ORM, Prisma, TypeORM, SQLAlchemy + asyncpg (Python), node-postgres (pg)). Apply its required settings (prepared statements, `pgbouncer=true` flags, pool sizes) to the generated config, and list them under *ORM Configuration Required* in the report.
...
### Known CVEs (verify live before finalizing)

The list below is a **starting point from early 2026, not a complete or current list**. Before finalizing the config:

1. Read the official changelog at https://www.pgbouncer.org/changelog.html (and the NVD entry for any CVE it names) for the version you are about to pin.
2. If you cannot browse, tell the user the CVE check is unverified and ask them to confirm the latest security release.
3. Record the checked version and date in the report's *Security* section.
...
- `references/orm-compatibility.md` — Driver-specific settings for Drizzle, Prisma, TypeORM, SQLAlchemy, and node-postgres
...
**Workflows:**
- **Connection Scaling Flow** (`db-scaling`, step 1 of 2): next → `@query-budget-enforcer` *(optional)*.

**Handoff contract:** pass results to the next skill through an inline *Handoff* block in your reply with the fields `skill`, `workflow`, `created_at`, `inputs`, `summary`, and `next` (the handoff contract of `@ecosystem-database`). If a next skill is not installed, continue with its step from the ecosystem map, or install it with `npx skills add fatih-developer/fth-skills --skill <name>` after the user agrees.
