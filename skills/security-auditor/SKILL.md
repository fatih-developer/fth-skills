...
Use the **Audit Report** template in `references/report-templates.md`.
...
Use the **Guard Protocols** template in `references/report-templates.md`.
...
Use the **Runtime Violations Ledger** template in `references/report-templates.md`.
...
Use the **Trust Score Report** template in `references/report-templates.md`.
...
Use the **Ecosystem Security Summary** template in `references/report-templates.md`.
...
- `references/report-templates.md` — Output templates for audit, guard, violation, trust, and ecosystem reports
...
**Direct handoffs:**
- `@skill-comparator` — A failing skill should be replaced by a safer alternative.

**Handoff contract:** pass results to the next skill through an inline *Handoff* block in your reply with the fields `skill`, `workflow`, `created_at`, `inputs`, `summary`, and `next` (the handoff contract of `@ecosystem-security`). If a next skill is not installed, continue with its step from the ecosystem map, or install it with `npx skills add fatih-developer/fth-skills --skill <name>` after the user agrees.
