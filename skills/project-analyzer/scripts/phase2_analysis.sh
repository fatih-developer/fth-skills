...
# usage of any (TypeScript quality indicator)
...
# Duplicate code block clue (similar function names)
...
echo "### 2.3 Güvenlik Analiz / Security Analysis"
# Hardcoded secret clue
...
# Insecure packages (known CVE version check)
...
echo "### 2.4 Performans Analiz / Performance Analysis"
# N+1 query risk (loop inside ORM)
...
try:
    ...
    for h in heavy:
        if h in deps: print(f'Heavy package: {h} {deps[h]}')
except Exception as e:
    pass
" 2>/dev/null
...
echo "### 2.5 Test Analizi / Test Analysis"
...
echo "### 2.6 API Tespiti / API Detection"
# REST endpoint detection — Express/Hono/FastAPI/NestJS
